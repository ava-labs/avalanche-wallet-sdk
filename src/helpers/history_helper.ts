...
import { ITransactionData, TransactionType, UTXO } from '@/History/types';
...
// Used with tokens
function addToDict(assetId: string, amount: BN, dict: TokenSummaryResult, utxo: UTXO, addresses: string[]) {
    if (dict[assetId]) {
...
function getNFTsSummary(tx: ITransactionData, walletAddrs: string[]): BaseTxNFTSummary {
    let nftLoss = getLossNFT(tx, walletAddrs);
    let nftGain = getGainNFT(tx, walletAddrs);
    return {
        sent: nftLoss,
        received: nftGain,
    };
...
function getLossNFT(tx: ITransactionData, walletAddrs: string[]): NFTSummaryResultDict {
...
function getGainNFT(tx: ITransactionData, walletAddrs: string[]): NFTSummaryResultDict {
    let addrsStripped = walletAddrs.map((addr) => addr.split('-')[1]);
...
function getLoss(tx: ITransactionData, walletAddrs: string[]): TokenSummaryResult {
    let ins = tx.inputs || [];
    let outs = tx.outputs;
...
function getProfit(tx: ITransactionData, walletAddrs: string[]): TokenSummaryResult {
    let outs = tx.outputs;
    let ins = tx.inputs || [];
...
async function getTransactionSummaryOld(tx: ITransactionData, walletAddrs: string[]): Promise<BaseTxSummary> {
    let losses = getLoss(tx, walletAddrs);
    let profits = getProfit(tx, walletAddrs);

    let nftSummary = getNFTsSummary(tx, walletAddrs);
...
export { getTransactionSummaryOld };
